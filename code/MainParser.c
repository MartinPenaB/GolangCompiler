...
extern gos_void startParser(gos_void);
extern int32 startScanner(BufferPointer sc_buf);

static gos_void printParserError(string fmt, ...);
static gos_void displayParser(BufferPointer ptrBuffer);
static gos_long getParserFileSize(string fname);
static gos_void callGarbageCollector(gos_void);
...
gos_void printParserError(string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(gos_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
gos_long getParserFileSize(string fname) {
	FILE* input;
	gos_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
gos_void displayParser(BufferPointer ptrBuffer) {
...
gos_void callGarbageCollector(gos_void) {
